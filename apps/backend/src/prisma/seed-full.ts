...
async function reset() {
  ...
  await prisma.payroll_documents.deleteMany({});
  await prisma.payroll_payments.deleteMany({});
  await prisma.professor_compensations.deleteMany({});
  ...
  await prisma.receipt_counters.deleteMany({});
...
async function main() {
        ...
        for (let g = 0; g < 3; g++) {
          ...
          for (let s = 0; s < 10; s++) {
            ...
            studentRows.push({
              ...
              status: (rand() > 0.95 ? "paused" : "active") as StudentStatus,
            });
  ...
  const crossFieldRows: any[] = [];
  const crossPaymentRows: any[] = [];
  const crossAssignmentRows: { student_id: string; group_id: string; fee: number }[] = [];

  // Group by level: level_id -> group[]
  const groupsByLevel: Record<string, typeof groupRows> = {};
  for (const g of groupRows) {
    const levelName = g.name.split(" ")[0] + " " + g.name.split(" ")[1]; // crude level prefix
    // find level from field -> prof -> group
  ...
  for (const student of studentRows) {
    ...
    const secondProf = profRows.find((p) => p.id === secondGroup.prof_id);
    const secondField = secondProf ? fieldRows.find((f) => f.id === secondProf.field_id) : null;
  ...
  if (crossAssignmentRows.length > 0) {
    await prisma.student_assignments.createMany({ data: crossAssignmentRows, skipDuplicates: true });
    await prisma.student_payments.createMany({ data: crossPaymentRows, skipDuplicates: true });
  ...
  for (const p of paidPayments) {
    ...
    transactionRows.push({
      ...
      compensation_snapshot: JSON.stringify({ model: "percentage", percentage: Math.round((profShare / amount) * 100) }),
      prof_id: profId,
      period: p.period,
    });
  }
  ...
  if (transactionRows.length > 0) {
    ...
    for (let i = 0; i < transactionRows.length; i += batchSize) {
      ...
      await prisma.payment_transactions.createMany({ data: batch, skipDuplicates: true });
    }
  }
