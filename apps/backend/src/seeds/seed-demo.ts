...
async function main() {
  ...
  const [existing] = await db.select({ count: sql<number>`count(*)::int` }).from(fields);
  if ((existing?.count ?? 0) > 0 && !shouldReset) {
    console.log(`Skipped: ${existing?.count} field(s) already exist. Re-run with --reset to rebuild.`);
    return;
  }
  ...
  const transactionRows: any[] = paymentRows
    ...
    .map((p) => {
      ...
      return {
        ...
        compensationModel: "percentage" as CompensationModel,
        compensation_snapshot: JSON.stringify({ model: "percentage", percentage: PROFESSOR_PERCENT }) as unknown,
  ...
  await db.insert(paymentTransactions).values(transactionRows).onConflictDoNothing();
