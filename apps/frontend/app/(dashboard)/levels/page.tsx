...
export default function LevelsPage() {
  ...
  const fieldForLevel = (level: Level): Field | undefined => {
    const profsForLevel = professors?.filter((p) => profFieldMap[p.id] && fields?.some((f) => f.id === profFieldMap[p.id]));
    const fieldIds = new Set<string>();
    professors?.forEach((p) => {
      const fid = profFieldMap[p.id];
      if (fid) fieldIds.add(fid);
    });
    return fields?.find((f) => fieldIds.size > 0);
  };
  ...
  const createMutation = useMutation({
    mutationFn: (data: { name: string }) => levelsApi.create(data),
  ...
  const fieldIdForLevel = (level: Level): string | undefined => {
    const profsForLevel = professors?.filter((p) => {
      const fid = profFieldMap[p.id];
      return fid !== undefined;
    });
    if (!profsForLevel || profsForLevel.length === 0) return fields?.[0]?.id;
    return profFieldMap[profsForLevel[0].id];
  };
  ...
  return (
    <>
                    ...
                    <tr key={level.id} className="hover:bg-background/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          {fid && (
                            <Link href={`/hierarchy/field/${fid}`} className="h-8 w-8 inline-flex items-center justify-center rounded-btn text-text-secondary hover:text-primary hover:bg-primary-50 transition-colors">
      ...
      <ConfirmDeleteDialog entityName={t("fieldsHierarchy.entityName")} isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={() => deleteId && deleteMutation.mutate(deleteId)} message={t("deleted.confirm", "It will be archived and can be restored later.")} />

      <DeletedEntities entityType="level" isOpen={deletedOpen} onClose={() => setDeletedOpen(false)} />
    </>
  );
}
